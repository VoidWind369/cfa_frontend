...
- **图标样式规则**：图标永远不加线框（border），仅使用纯颜色显示
- **统计逻辑**：
  - 以当前查询部落为基准判断胜负视角：`self_clan_id === id` 时 result 不变，否则 result 取反（1→-1, -1→1）
  - `win` = 查询部落为赢的记录（含奖励局赢的）
  - `lose` = 查询部落为输的记录（含惩罚局输的）
  - `award` = type===11 且查询部落为赢（奖励局赢）
  - `penalty` = type===12 且查询部落为输（惩罚局输）
  - `other` = result 为 0（未结算）
- **列表视角**：列表不做视角转换，直接使用 API 返回的 self/rival 原始数据
