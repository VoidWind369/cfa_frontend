...
import { ArrowLeft, Save, Trash2, Trophy, Skull, AlertTriangle, Plus, Minus, Clock } from 'lucide-react';
...
const ClanEditPage = () => {
  ...
  const handleDelete = async () => {
    if (!clan || !isAdmin) return;
    if (!confirm(t('clan.confirm_delete'))) return;
    setSaving(true);
    try {
  ...
  return (
    <>
      <header className="mb-8 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-brand-text">{clan.name || '...'}</h1>
